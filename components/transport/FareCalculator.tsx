...
function isCommuterType(
  value: string,
): value is CommuterType {
  return (
    value === "regular" ||
    value === "discounted"
  );
...
export function FareCalculator({
  ...
  const selectedFare = fareRules.find(
    (rule) =>
      rule.routeId === routeId &&
      rule.commuterType === commuterType,
  );
  ...
  if (routes.length === 0) {
    return (
      <section
        id="fare-calculator"
        aria-labelledby="fare-calculator-heading"
        className="rounded-2xl border border-tourism-border bg-white p-5 shadow-sm sm:p-6"
      >
      ...
      {selectedRoute && selectedFare ? (
        ...
        >
          ...
          <p className="mt-2 text-3xl font-black tracking-tight text-tourism-navy">
            {formatPeso(selectedFare.amount)}
          </p>
