  ...
  const filteredDestinations = useMemo(() => {
    ...
    return destinations.filter(
      (destination) => {
        ...
        const matchesQuery =
          !normalizedQuery ||
          `${destination.name} ${destination.category} ${destination.location} ${destination.description}`
            .toLowerCase()
    ...
    >
      <div className="rounded-2xl border border-tourism-border bg-white p-4 shadow-sm sm:p-5">
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <label className="flex min-h-11 items-center gap-3 rounded-xl border border-tourism-border bg-tourism-surface px-4 transition focus-within:border-tourism-pink focus-within:ring-2 focus-within:ring-tourism-pink/30 motion-reduce:transition-none">
            <Search
              aria-hidden="true"
          ...
          >
            <button
              ...
              className={[
                "shrink-0 rounded-full px-4 py-2 text-[10px] font-bold transition",
      ...
      <p
        aria-live="polite"
        className="mt-6 text-sm text-tourism-muted"
      >
        {filteredDestinations.length}{" "}
        destination
